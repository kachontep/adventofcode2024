#
# Source from: https://winslowjosiah.com/blog/2024/12/23/advent-of-code-2024-day-21/
#
# Duplicate from the blog to learn and study how to solve this problem.
# I confess that intially I didn't think this problem must search all possible paths for two buttons
# 
import sys
...
def get_button_paths(pad: Pad, button1: str, button2: str) -> list[str]:
    ...
    movements = "<" * -button_dc + ">" * button_dc + "^" * -button_dr + "v" * button_dr
...
def get_min_sequence_length(pad: Pad, code: str, depth: int) -> int:
    ...
    for button1, button2 in it.pairwise("A" + code):
        ...
        # If we're controlling some directional keypad
            ...
            result += min(
                get_min_sequence_length(DIRPAD, path, depth - 1) for path in paths
            )
    return result
...
def aoc2024_day21_part1(lines: Iterable[str]) -> int:
    ...
    for keycode in lines:
        ...
        print(min_sequence_length, keycode_number, min_sequence_length * keycode_number)
...
def main():
    ...
    print(aoc2024_day21_part1(lines))
